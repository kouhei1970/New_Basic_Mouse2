...
)
{
	...
	for (;;) {
		...
		for (w = 0; c >= '0' && c <= '9'; c = *fmt++)	/* Minimum width */
			w = w * 10 + c - '0';
		wp = 6;
		if (c == '.'){				/* After the decimal point width */
			c = *fmt++;
			for (wp = 0; c >= '0' && c <= '9'; c = *fmt++)
				wp = wp * 10 + c - '0';
		}
		...
		if(f & 16){
			...
			v = (unsigned long)vf;
			vf = vf - (float)v;
			for(i = 0; i < wp; i++) vf *= 10.0;
			vd = (unsigned long)(vf + 0.5);
			i = 0;
			if(wp > 0){
				do {
					d = (char)(vd % r); vd /= r;
					s[i++] = d + '0';
				} while (i < wp && i < sizeof(s));
				s[i++] = '.';
			}
			do {
				d = (char)(v % r); v /= r;
